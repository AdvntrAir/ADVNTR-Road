...
Before rendering anything, read **`ADVNTR_Road_Claude_Code_Build_Instructions_v2_0.md`** from the
repo — input contract, schema 3.0 renderer expectations, v2.0 components, QR system, the Gate 3
QA checklist, build summary format.

For everything else, load ONLY the split spec file relevant to the task at hand:
...
**NEVER load `ADVNTR_Road_Build_Spec_FINAL.md` directly.** That file is 112KB. Loading it caused
16.7M cache-read tokens and ~$4 per session. Read spec docs once at session start — do not
re-read them mid-session.

**This skill deliberately does not restate spec contents.** The QA checklist, the schema, the
